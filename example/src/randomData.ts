...
const addr = () =>
  `${randInt(100, 9999)} ${pick(streets)}, ${pick(cities)} ${randInt(
    10000,
    99999
  )}`;
const phone = () =>
...
const email = (n: string) =>
  `${n
    .toLowerCase()
    .replace(/[^a-z]/g, '')
    .slice(0, 8)}@${pick(domains)}`;
const companyEmail = (c: string) =>
  `${pick(['billing', 'info', 'hello', 'contact', 'support'])}@${c
    .toLowerCase()
    .replace(/[^a-z]/g, '')}.com`;
const invoiceNum = () =>
  `INV-${new Date().getFullYear()}-${String(randInt(1, 999)).padStart(3, '0')}`;
const receiptNum = () =>
  `R-${Date.now().toString().slice(-8)}-${String(randInt(1, 999)).padStart(
    3,
    '0'
  )}`;
...
const dateTimeStr = () => {
  ...
  return `${d.toISOString().slice(0, 10)} ${String(d.getHours()).padStart(
    2,
    '0'
...
export function randomInvoiceData() {
  ...
  const discount = randInt(0, 1)
    ? Math.round(subtotal * pick([0.05, 0.1, 0.15]) * 100) / 100
    : 0;
  ...
  return {
    data: {
      ...
      paymentTerms: `Bank transfer to ${comp}, Account: ${randInt(
        1000,
        9999
...
export function randomReceiptData() {
  ...
  const subtotal = items.reduce(
    (s, i) => s + i.quantity * parseFloat(i.price.replace(/[$,]/g, '')),
    0
  );
    ...
    data: {
      ...
      sections: [
        ...
        { heading: 'Timeline', content: timelineTexts() },
        {
          ...
          content:
            'All intellectual property rights in the deliverables shall transfer to the Client upon full payment. The Developer retains the right to use general knowledge and techniques gained during the project.',
        },
        {
          heading: 'Confidentiality',
          content:
            'Both parties agree to maintain the confidentiality of all proprietary information exchanged during the term of this agreement and for a period of two years following its termination.',
        },
      ],
...
export function randomLetterData() {
  ...
  return {
    data: {
      ...
      greeting: `Dear ${pick(['Mr.', 'Ms.', 'Dr.'])} ${rName
        .split(' ')
        .pop()},`,
